...
| Studio (public profile) | `components/studio/studio.css` (`.pq-studio-*` top: cover, head, avatar, actions with `:has` phone rule, metrics, About, private card, tabs, loading), `StudioHeader.tsx` (identity + actions + counts + About as one component with props for every state), `socialIcons.tsx`, `FollowersModal` on `Sheet` + `PersonRow`; block / report / communities on the overlay layer. Content views below the tabs are 6B |
