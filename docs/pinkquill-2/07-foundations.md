...
| Takes (feed, page, modal, editors) | `components/takes/takes.css` (all Takes CSS; nothing left in `globals.css`), `TakeStage.tsx` (shared video stage), `TakeEditorPieces.tsx` (shared editor controls), `TakeCommentsPanel.tsx` on `Sheet` (`presentation="panel"`, `initialFocus`). PostDetail seams accept `typeLabel`, `reactionControl`, `thread`/`count` for non-post works |
| Share | `components/ui/ShareModal.tsx` on `Sheet` + `TabRow`; `.pq-share*` in `overlay.css`; story-image preview classes stay in `globals.css` |
| Communities | `lib/communities/categories.ts` (the one taxonomy: categories, genres, themes, purposes, count helpers), `components/communities/communities.css` (directory, card, identity, home layout, create rows), `CommunityMark`/`membershipWord` in `CommunityCard.tsx`, `JoinButton` on Button/Sheet/ConfirmationModal, `TimeRangeDropdown` on ActionMenu; the slug layout client owns identity, tabs and the three gates |
| Community staff surfaces | `components/communities/pieces.tsx` (`PersonRow`, `CommunitySettingsFrame` + `settingsSections`, `Notice`), `ModerationSheet` (mute/ban), `InviteModal` and `ModeratorPermissionsModal` on `Sheet`, `ModQueue/*` on shared cards and choices; reusable by studio followers lists and admin console people rows |
| Messages and notifications | `components/messages/messages.css` (`.pq-msgs` frame with `data-open`, `.pq-thread-row`, `.pq-chat-*`, `.pq-bubble`, `.pq-chat-compose`, `.pq-voice*`), `components/notifications/notifications.css` (`.pq-notif*`); `NewMessageModal` and `SendToDMModal` on `Sheet` + `PersonRow`; `ChatView` report on `ReportModal`, block and delete on `ConfirmationModal`; the community inbox reuses every DM class |
| Studio (public profile) | `components/studio/studio.css` (`.pq-studio-*` top, works bar, pin rows, `.pq-collection*`, `.pq-crumbs`, item page), `StudioHeader.tsx`, `StudioWorks.tsx` + `works.ts` (kinds, pinned order) on the shared `GalleryFeed` / `StreamFeed`, `PinSheet.tsx`, `StudioRelays.tsx`, `CollectionCard.tsx` (`CollectionMark`), `FollowersModal` on `Sheet` + `PersonRow`; `StudioProfile.tsx` is hooks and composition only. `lib/feed-view/transform.ts` is the one Post to card transformer for feed, studio, relays and collections |
| Saved library | `components/saved/SavedLibrary.tsx` + `words.ts` + `saved.css`; posts on `GalleryFeed`, takes on the `TakePostCard` grid, products on a stand-in tile until 7A; removal is un-saving (the tile bookmark for posts through the modal's save updates, one icon button for takes and products) |
