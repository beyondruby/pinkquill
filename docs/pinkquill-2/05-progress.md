...
## Exact next task: 5A communities discovery and community home

Read `03-phases.md` §5 and the 4C entry above. Redesign the Communities directory (`/communities`: cards, categories, joined/created views, create flow) and the community home (`/communities/[slug]`: identity header, contribution entry, post feed on the 3B card language, pinned content, sorts). Reuse `PageFrame`/`PageHeader`, `TabRow`, chips, list rows, the feed views and `Discussion`/PostDetail seams; keep the word Communities; no fictional renaming. Move community CSS out of `globals.css` into `components/communities/` as it is touched (same rule as Takes). Verify sparse and busy communities, member and non-member states.
