...
## Exact next task: 4C Takes and pending collaborations

Read `03-phases.md` §4C and the 4B entry above. Redesign Takes: `/takes` feed (`components/takes/TakePostCard.tsx`, `TakeCard.tsx`, reaction picker and comments panel layering), `/take/[id]` and `TakeDetailModal` (reuse the PostDetail seams where behaviour matches: creator row, action row, Discussion; move the Take modal off `.view-discussion-btn`/`.discussion-panel` and delete those rules), `/takes/create` (`CreateTake.tsx`) and the Take editor block inside `CreatePost.tsx` (dropzone, filters, sound, ratio, cover onto composer classes), and `/pending-collaborations`. Keep playback controls visible over varied footage, reachable on phones, separate from global navigation; keep every overlay above video via the overlay layer. Also migrate `ShareModal` and `SendToDMModal` onto `Sheet` as they are touched.
