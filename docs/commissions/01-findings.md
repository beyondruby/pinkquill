...
10. **A paid commission is marked delivered and paid out instantly.** The webhook's instant-delivery branch keys on `products.delivery_type === 'digital'` alone (`webhooks/route.ts:180-186`); the listing wizard stores `delivery_type = 'digital'` for every service, so the moment a commission is paid the webhook calls `transferToSeller` and sets `status = 'delivered'` — the buyer sees "Delivered / Confirm Receipt / auto-completes in 3 days" before the seller has done anything, and an onboarded seller is paid before delivering. **Confirmed live in Stripe test mode on 2026-09-02** (transfer attempt recorded as `pending_onboarding`; see `03-progress.md`). The same run confirmed the duplicate `order_paid` seller notification (item 7) and the double session creation (item 9: two sessions in the same second for one order).
