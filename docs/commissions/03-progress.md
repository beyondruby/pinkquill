...
| Follow-up — request sheet sizing + tax invoice PDF | **done, committed on main**; migration `commissions_invoice_issuer` applied to prod | 2026-09-03 |
...
**Next (at the time):** 2f. Done below.

---

## Phase 2f — Operations (2026-09-03)

**Closes:** the 2f line of the plan and D8's console — `/admin` with orders search, refunds needing review, failed payouts, disputes with evidence and resolution, chargeback evidence submission to Stripe, a platform settings editor and cron health. Finding 6's "no admin" gap (RC-B1) is closed; tax collection stays open.

**No mockup** (Phase-2 rules). The money workers are untouched: the refund executor, payout worker and webhooks still do every transfer. "Retry" only puts a row back into the state those workers pick up.

**Migration** `supabase/migrations/20260903_commissions_phase2f_operations.sql` (applied to prod as `commissions_phase2f_operations`). Eight service-role-only RPCs, each taking the acting admin's id and verifying it against `platform_admins` inside (the API routes verify the session first): `admin_log` (audit row into `ops_alerts`, kind `admin_action`, severity `info`, resolved on insert — no new table), `admin_resolve_alert`, `admin_retry_payout` (failed/held → pending, attempts reset, refused while a dispute is still open), `admin_unblock_seller_payouts`, `admin_retry_refund` (needs_review/failed → approved), `admin_cancel_refund` (unexecuted refund → cancelled; a full refund that had cancelled a still-paid order restores the order to its previous status, with a thread message), `admin_update_setting` (allow-listed keys with ranges: fee rates 0–0.3, money 0–100, hold ≤ 720 h, whole numbers 1–100, `settlement_currency` cad|usd, `app_base_url` https origin, `supported_currencies` three-letter list; the old and new value go into the audit row), `admin_search_orders` (one query across order number, listing title, buyer and seller name, payment intent, session and order id, optional status, ≤ 200 rows). Plus `run_admin_selftest()` (rolled back): non-admin refused; failed payout retried once and not twice; needs_review refund retried then cancelled with the order restored to `paid`; bad rate and unknown key refused, good value applied; search by number and by seller + status; four audit rows. Added to `lib/__tests__/db-selftests.test.ts` (now six DB tests).

**API** under `app/api/admin/*`, all behind `requireAdmin` (session + `platform_admins`) and same-origin on writes: `orders` (search), `refunds` (list; retry runs the existing `executeApprovedRefunds` once for that order, like the disputes route already did; cancel), `payouts` (list with each seller's Stripe account state; retry; unblock), `disputes` (extended with `?scope=all`), `disputes/[id]` (the pack: order, both people, message thread, deliveries with file counts, evidence items, and for chargebacks a pre-filled draft of Stripe's text fields), `chargebacks` (evidence → Stripe), `settings` (get; validated update), `system` (cron schedule + last 40 runs, failed/stuck Stripe events, open alerts, audit history; run a job now; resolve an alert), `files` (5-minute signed URL for an evidence file — operators are not order participants, so `/api/orders/files` would refuse them). Shared helpers in `lib/admin-server.ts`.

**Chargebacks.** `lib/admin/chargeback-evidence.ts` builds Stripe's text fields from the order: what was sold (listing, package, brief, price), the message thread as a transcript, a timeline of delivery versions and approval, our account of what happened (including the participants' evidence statements), service date, customer name and email, and the refund and cancellation policy disclosures; every field is editable in the console and clipped to Stripe's limits. Files come only from the dispute's own evidence attachments, one per Stripe file field (uncategorized, customer communication, service documentation, receipt, policies, signature, shipping), read from the private bucket and uploaded as `dispute_evidence`. **Save draft to Stripe** updates the dispute without submitting; **Submit to card network** submits (confirmed in a sheet, because Stripe accepts exactly one submission). The route records an admin evidence item with the Stripe status, logs the action, and refuses anything that is not an open chargeback. `dry_run: true` returns the payload without touching Stripe. Unit-tested (`lib/__tests__/chargeback-evidence.test.ts`).

**Console** `app/admin/*` on `components/admin/AdminShell.tsx` (app rail + section list, phone tab strip; non-admins get a plain "for operators" page, never data): **Overview** (refunds to review, payouts failed/held, disputes open with chargebacks, alerts open with cron state, orders active, Stripe events, FX age, owed to sellers from the ledger; open alerts; cron jobs with last result), **Orders** (search + status filter, dispute/refund flags, payment state, payout state, receipt link), **Refunds** (needs a person / everything; Retry, Cancel refund with a note in a sheet, Approve-for-seller / Decline on requests — through the existing `decide_refund_request`, which already admits admins), **Payouts** (not sent yet / everything; seller's Stripe readiness and requirements; Retry, Unblock seller), **Disputes** (open / everything; kind, reason, amount, evidence count, evidence-due chip that turns red under three days) and **Dispute detail** (case, evidence with file links, collapsible thread, order box; platform disputes resolve through the 1d RPC with a confirmation sheet; chargebacks get the evidence editor), **Settings** (every platform setting with a label and a plain-language help line, inline edit + Save, recent changes), **System** (cron jobs with Run now, recent runs, Stripe events needing attention, open alerts with Resolve, history = resolved alerts and every console action). Routine actions use an arm-then-confirm button; irreversible ones use a sheet.

**Code:** `lib/admin-server.ts`, `lib/admin/chargeback-evidence.ts`, `lib/admin/dispute-pack.ts`, `lib/hooks/useAdmin.ts` (`adminFetch`, `useIsPlatformAdmin`, `useAdminQuery`), `components/admin/*` (Shell, ui, Overview, Orders, Refunds, Payouts, Disputes, DisputeDetail, Settings, System), `app/admin/*` pages and layout, nine routes under `app/api/admin/`.

**Verified**
- `npx tsc --noEmit` clean; ESLint 0 errors on every changed file; `npx vitest run` 166 pass (3 new evidence tests); `RUN_DB_SELFTEST=1 …` six DB tests pass (money, listing, workroom, timeline, analytics, admin); `npm run build` succeeds and lists every `/admin` and `/api/admin` route.
- Browser (local dev server, signed in as `hadi`, the platform admin; with a temporary completed order carrying a payment row and a fake chargeback created through `record_chargeback`): Overview shows the live counts (1 chargeback, cron ok, FX 1.3925 · 269 min old, ledger empty); Orders lists both orders with "Dispute open · dispute" flags on the test one and a receipt link; the chargeback detail renders the case, the order box, the two-message thread and the pre-filled Stripe fields (What was sold, Message thread transcript with timestamps, timeline, our account, service date, customer, policy disclosures); a `dry_run` POST to the chargeback route from the page returns the payload with the unknown key dropped and the Stripe dispute id; Settings shows all 13 settings with help lines, and saving `platform_fee_rate = 5` is refused with "must be between 0 and 0.3 (a rate, not a percent)" — nothing written; System lists the three cron jobs, 40 recent runs and no Stripe events; **Run now** on auto-decline ran it (`{"declined":0}`) and wrote one audit row. No console errors. The test order, payment, dispute, notifications and alert rows were deleted afterwards (production: 1 real order; the one audit row from the cron run stays, it is a real action).
- **Not exercised in the UI:** refund retry/cancel and payout retry (self-test only — production has none to act on), a real evidence upload and submission to Stripe (needs a real dispute; the route is exercised up to the Stripe call by the dry run and the unit tests), the non-admin page, phone layouts.

**Deferred**
- Chargeback outcomes still arrive only through the webhook (`charge.dispute.closed`); the console shows `stripe_status` but does not poll Stripe.
...
## Follow-up — request sheet sizing and a real tax invoice PDF (2026-09-03)

Two reports after the rebuild.

**1. "The request modal is too big and you can't scroll."** `Sheet` gave `size="tall"` a fixed height (94vh on phones, 85vh from md). On a phone the browser's own toolbar eats into `vh`, so with page scroll locked the footer buttons sat below the visible area and nothing could scroll to them; on desktop the dialog was a mostly empty 85vh box. The panel is now content-sized and capped by the *dynamic* viewport (`.sheet-panel` in `globals.css`: 90dvh on phones, 85dvh from md; `tall` keeps a 440px floor on desktop so the four steps don't jump), the body scrolls inside with `overscroll-contain`. Measured after the change: a 440px dialog for the package step with a 732px cap on an 862px viewport.

**2. "Where are the tax invoices I can download as PDF?"** The 2e receipt page existed, but "Download PDF" was the browser's print dialog and nothing called it an invoice. Now:
- `GET /api/orders/[id]/invoice` renders a real PDF on the server: Letter page, PinkQuill wordmark, "TAX INVOICE", invoice number (= order number), issue date and status, Billed to / From (issuer block from the new `invoice_issuer` platform setting, "On behalf of @creator"), the line item with package details, subtotal, shipping, discount, processing fee, a Tax line (0.00 with the issuer's tax note — no tax is collected), Total paid, the card charge in CAD with the day's rate, payment method / date / Stripe reference, refunds, and a footer. Participants or operators only; unpaid orders get a 409; anonymous a 401. Served inline (opens in the browser) with a filename for saving.
- `lib/pdf/pdf-writer.ts` is a small dependency-free PDF writer (Letter, Helvetica + Helvetica-Bold with their real advance widths for right-aligned money, rectangles, lines, word-wrap) so nothing has to be bundled for serverless. `lib/invoice/render-invoice.ts` is the layout; unit-tested, and the test writes a sample the reviewer can render with `pdftoppm`.
- The receipt page is now titled **Tax invoice** with a Tax row, and its **Download PDF** button is a real download of that PDF (the payout statement keeps print-to-PDF). The order page's Summary link reads **Invoice · PDF**; buyer order cards show an **Invoice** link on paid orders; the admin orders row says invoice.
- `platform_settings.invoice_issuer` (`{name, lines[], tax_note}`) is editable from the console (JSON field; `admin_update_setting` gained a validated branch, edited in place from its live definition).

**Verified:** `npx tsc --noEmit` clean; ESLint 0 errors on the changed files and an errors-only sweep of the marketplace code; `npx vitest run` 190 pass (3 new: writer well-formedness incl. xref offsets, WinAnsi sanitising and Helvetica widths, the rendered invoice); `npm run build` succeeds. Browser: the request sheet measured content-sized with the dvh cap; the real order's invoice returned 200 / `application/pdf` / 4.6 KB starting with `%PDF-`, opened in Chrome's viewer, and the route answers 401 anonymously; the sample PDF rendered by `pdftoppm` was eyeballed (header, parties, table, totals, payment, footer all in place).
