    ...
    <div className={isReply ? "ml-11 mt-3" : ""}>
      ...
      {showReportModal && (
        <>
          ...
          />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] bg-white rounded-2xl shadow-2xl z-[61] p-5">
            {reportSubmitted ? (
...
export default function TakeCommentsPanel({ isOpen, onClose, takeId }: TakeCommentsPanelProps) {
  ...
  return (
    <>
      ...
      {/* Panel - matches discussion-panel styling */}
      <div className={`fixed top-0 right-0 h-full w-[380px] bg-[#fafafa] z-50 flex flex-col transform transition-transform duration-300 ease-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        {/* Input - matches feed comment input styling */}
        {user ? (
          <div className="p-4 bg-white border-t border-black/[0.06] flex gap-2.5 items-center">
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex items-center gap-2.5">
