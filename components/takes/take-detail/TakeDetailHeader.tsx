...
export function TakeDetailHeader({ take, actions, discussion, className = "" }: Props) {
  ...
  return (
    <div className={`take-detail-header flex items-center gap-3 md:gap-4 ${className}`}>
      <Link href={`/studio/${take.author.username}`} onClick={onNavigate} className="flex-shrink-0">
        <Image
          ...
          width={56}
          height={56}
          className="w-12 h-12 md:w-14 md:h-14 rounded-full object-cover border-2 md:border-[3px] border-white shadow-lg hover:scale-110 transition-transform"
          sizes="56px"
          quality={80}
        />
      </Link>
      <div className="flex flex-col gap-0.5 md:gap-1 flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <Link href={`/studio/${take.author.username}`} onClick={onNavigate} className="font-ui text-[1rem] md:text-[1.1rem] font-medium text-ink hover:text-accent transition-colors truncate max-w-full">
            {name}
          </Link>
          <span className="font-ui text-[0.85rem] md:text-[0.9rem] font-light text-muted">shared a take</span>
        </div>
        <span className="font-ui text-[0.8rem] md:text-[0.85rem] text-muted">{getTimeAgo(take.created_at)}</span>
      </div>
