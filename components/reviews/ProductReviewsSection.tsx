...
import ReviewCard, { QuillMeter, QUILL_TONE } from "./ReviewCard";
...
export default function ProductReviewsSection({ productId }: ProductReviewsSectionProps) {
  ...
  return (
    <section className="pt-8 border-t border-border-light">
      {/* Section heading — matches the editorial headings above (Specifications, Shipping…) */}
      <div className="flex items-center justify-between gap-4">
        ...
        {!loading && !error && reviews.length > 0 && (
          <span className="inline-flex items-center gap-1.5 text-[11px] font-body text-muted">
            ...
            Verified from completed orders
          </span>
        )}
      </div>

      {/* Aggregate summary — a calm display number, no gradient blobs */}
      {average > 0 && (
      ...
      {!loading && !error && reviews.length === 0 && (
        <div className="mt-5 rounded-2xl border border-border-light bg-subtle/40 px-6 py-10 text-center">
          ...
          <p className="text-sm font-body text-muted">
            No reviews yet — they appear here once an order is completed.
          </p>
        </div>
      )}
