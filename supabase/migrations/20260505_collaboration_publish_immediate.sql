-- Collaboration flow change: posts publish immediately, even when collaborators
-- are invited. A collaborator accepting only causes the post to surface on
-- their profile (via fetchCollaboratedPosts, which filters by accepted status).
--
-- Replaces:
--   - create_post_with_relations RPC (no longer flips status to 'draft')
--   - auto_publish_post trigger (no longer needed; post is already published)

DROP TRIGGER IF EXISTS trigger_auto_publish_post ON post_collaborators;
DROP FUNCTION IF EXISTS auto_publish_post();
...
-- Backfill: any existing posts still in 'draft' status because they had
-- pending collaborators under the old flow should now be published.
-- (Safe scope: only posts that have at least one collaborator row.)
UPDATE posts p
SET status = 'published'
WHERE p.status = 'draft'
  AND EXISTS (
    SELECT 1 FROM post_collaborators pc WHERE pc.post_id = p.id
  );
