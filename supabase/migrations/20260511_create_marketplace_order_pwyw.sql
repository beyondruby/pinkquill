-- Extend create_marketplace_order to support PWYW: buyer-chosen amount
-- replaces the static price for pricing rows where min_price < price.
-- Fixed-price rows (min_price = price) ignore p_chosen_amount.

-- Drop the prior 8-arg signature to remove dispatch ambiguity.
DROP FUNCTION IF EXISTS public.create_marketplace_order(
  UUID, UUID, UUID, INTEGER, TEXT, JSONB, TIMESTAMPTZ, JSONB
);
...
CREATE OR REPLACE FUNCTION public.create_marketplace_order(
  ...
  p_chosen_amount NUMERIC DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE id = p_pricing_id AND product_id = p_product_id
  FOR UPDATE;
  ...
  IF v_is_pwyw THEN
    ...
    IF v_unit_price > 1000000 THEN
      RAISE EXCEPTION 'Chosen amount % exceeds the maximum allowed', v_unit_price;
    END IF;
  ELSE
  ...
  v_platform_fee := ROUND((v_item_amount * 0.05)::NUMERIC, 2);
  ...
  INSERT INTO orders (
    ...
    platform_fee, seller_amount, currency,
  ...
  VALUES (
    ...
    v_amount, v_amount, 0, NULL,
    v_platform_fee, v_seller_amount, v_currency,
    v_initial_status, 'pending', 'placeholder',
  ...
  VALUES (
    v_order_id, p_buyer_id, 'status_change', NULL, v_initial_status,
    jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION create_marketplace_order(UUID, UUID, UUID, INTEGER, TEXT, JSONB, TIMESTAMPTZ, JSONB, NUMERIC) TO service_role;
