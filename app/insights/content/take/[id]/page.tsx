...
import { useTakeInsights, TimeRange, DateRange } from "@/lib/hooks/useInsights";
...
export default function TakeInsightsPage() {
  const params = useParams();
  const takeId = params.id as string;
  const [timeRange, setTimeRange] = useState<TimeRange>("30d");
  const [customRange, setCustomRange] = useState<DateRange | undefined>();

  const { insights, take, loading, error } = useTakeInsights(takeId, timeRange, customRange);
    ...
    return (
      <div>
        ...
        <EmptyState
          ...
          description={error || "This take could not be found or you don't have permission to view its insights."}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <MetricCard
          label="Avg. Loops"
          value={insights.avgLoopCount.toFixed(1)}
          description="Replays per viewer"
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <MetricCard
          ...
          value={formatDuration(insights.totalWatchTime || 0)}
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <MetricCard
          ...
          description="Times shown in feed"
          icon={
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
