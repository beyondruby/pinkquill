...
// ============================================================================
// useTrendingTags HOOK - For trending hashtags sidebar
// ============================================================================
...
export function useTrendingTags(limit: number = 10) {
  ...
  useEffect(() => {
    const fetchTrendingTags = async () => {
      try {
        // Get tags from recent posts (last 7 days)
        ...
        const { data, error } = await supabase
          ...
          .select(`
            tag:tags (name),
            post:posts!inner (created_at)
          `)
          .gte("post.created_at", sevenDaysAgo.toISOString());
